...
/** @param {number} value @param {number} shift */
function rotateRight(value, shift) {
  return (value >>> shift) | (value << (32 - shift));
}

/** UTF-8 SHA-256 digest bytes. @param {string | Uint8Array} input */
export function sha256Bytes(input) {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    for (let index = 0; index < 16; index += 1) schedule[index] = paddedView.getUint32(offset + index * 4);
...
/** Lowercase hex encoding. @param {Uint8Array} bytes */
...
/** UTF-8 SHA-256 digest as lowercase hex. @param {string | Uint8Array} input */
